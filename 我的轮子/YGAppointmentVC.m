...
- (void)addContentView {
    ...
    NSString *file = [NSHomeDirectory() stringByAppendingPathComponent:@"Documents/tempPath"];
    ...
    file = [file stringByAppendingPathComponent:@"cs.silk"];
    
    NSString *newFile = [file stringByAppendingPathComponent:@"cs.pcm"];
//    NSData *info = [NSData dataWithContentsOfFile:file];
//    NSMutableData *data = [NSMutableData data];
//    [data appendData:[info subdataWithRange:NSMakeRange(1, info.length-1)]];
//    NSString *str = [[NSString alloc] initWithData:data encoding:NSASCIIStringEncoding];
    
//    char* a=[data bytes];
//    const char * a =[str UTF8String];
//    info.bytes
    char *buff[3]= {[file UTF8String],[file UTF8String],[newFile UTF8String]};
    int rets = [self setDecoder:3 argv:buff];
    NSLog(@"%d",rets);
////    NSLog(@"%d",rets);
//    void *psDec;
...
//////
//////    
//    SKP_int16 output_buffer;
//    SKP_int16 len;
////    SKP_int16 tot_len;
//    ret = SKP_Silk_SDK_Decode( psDec, &DecControl, 0,(SKP_uint8 *)[str UTF8String], (SKP_int)100, &output_buffer,&len );
//    NSLog(@"%d",ret);
//    char *fss = [info bytes];
//    char *cStrArr = [str cStringUsingEncoding:NSASCIIStringEncoding];
//    char *rrrr[1];
//    int rets = decoder(1, &rrrr[1]);
//    NSLog(@"%d",rets);
//    int rets = getDecoder(1, [file UTF8String]);
//    fwrite( out, sizeof( SKP_int16 ), tot_len, speechOutFile );
//    [str cStringUsingEncoding:1];
//    FILEFILE *pcm = fopen([str cStringUsingEncoding:1], "rb");  //source 被转换的音频文件位置
//    fseek(pcm, 4*1024, SEEK_CUR);                                   //skip file header
//    FILEFILE *mp3 = fopen([mp3FilePath cStringUsingEncoding:1], "wb");  //output 输出生成的Mp3文件位置
//    unsigned char ssss = [str cStringUsingEncoding:1];
//    int i = SKP_Silk_SDK_InitDecoder((__bridge void *)(@1));
//    i = SKP_Silk_SDK_Decode(nil, nil, 0, &ssss, info.length, 1, 1);
//    NSLog(@"%@",i);

//    for(int i = 0; i < 100; i++ ) {
//        ret = SKP_Silk_SDK_Decode( psDec, &DecControl, 1, (SKP_uint8 *)info.bytes, (SKP_int)info.length, &output_buffer, &len );
//        if( ret ) {
...
- (int)setDecoder:(int)argc argv:(char *)argv {
