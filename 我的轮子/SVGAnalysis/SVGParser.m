...
NSInteger const df		= 1;
NSInteger const dg		= 0;

NSString* const separatorCharString = @"-, CcMmLlHhVvTtZzqQaAsS";
NSString* const commandCharString	= @"CcMmLlHhVvTtZzqQaAsS";
...
{
    SVGParser *pocketSVG = [SVGParser pocketSVGFromSVGPathNodeDAttr:[self parseSVGNamed:nameOfSVG]];
    
#if TARGET_OS_IPHONE
    
    pocketSVG.bezier.flatness = 1.0;
...
{
    SVGParser *pocketSVG = [SVGParser pocketSVGFromSVGPathNodeDAttr:dAttribute];
#if TARGET_OS_IPHONE
    return pocketSVG.bezier.CGPath;
#else
    return [PocketSVG getCGPathFromNSBezierPath:pocketSVG.bezier];
...
+ (SVGParser*)pocketSVGFromSVGPathNodeDAttr:(NSString *)attr
{
    SVGParser *pocketSVG = [[SVGParser alloc] initFromSVGPathNodeDAttr:attr];
...
- (void)appendSVGMCommand:(Token *)token
{
    ...
    CGFloat x = [token parameter:index++]/df+dg + (isRelative ? lastPoint.x : 0);
    CGFloat y = [token parameter:index]/df+dg + (isRelative ? lastPoint.y : 0);
    ...
    while (index < [token valence]) {
        ...
        switch ( [token command] ) {
            ...
            case 'L':
                ...
                x += ([token parameter:index++]/df+dg);
                y += ([token parameter:index++]/df+dg);
                break;
            case 'h' :
                x = lastPoint.x;
            case 'H' :
    ...
    while (index < [token valence]) {
        ...
        switch ([token command]) {
            ...
            case 'Q' :
                ...
                x1 += [token parameter:index++]/df;
                y1 += [token parameter:index++]/df;
                x += [token parameter:index++]/df;
                y += [token parameter:index++]/df;
                break;
            default:
...
- (void)appendSVGCCommand:(Token *)token
{
    ...
    CGFloat x1 = [token parameter:index++]/df + (isRelative ? lastPoint.x : 0);
    CGFloat y1 = [token parameter:index++]/df + (isRelative ? lastPoint.y : 0);
    CGFloat x2 = [token parameter:index++]/df + (isRelative ? lastPoint.x : 0);
    CGFloat y2 = [token parameter:index++]/df + (isRelative ? lastPoint.y : 0);
    CGFloat x  = [token parameter:index++]/df + (isRelative ? lastPoint.x : 0);
    CGFloat y  = [token parameter:index++]/df + (isRelative ? lastPoint.y : 0);
    lastPoint = CGPointMake(x, y);
#if TARGET_OS_IPHONE
